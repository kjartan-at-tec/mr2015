...
%% Feedforward from disturbance flow out of tank 2

% For cascade controller

% Fv = -G2^{-1}Hv = -(1/(s tau2 + 1))^{-1} (-K1/s tau1+1)
%    = K1 (s tau2 + 1)/(s tau1 + 1) \approx K1
Fv1 = K1;

% For SIMC controller

% Fv = -G^{-1}Hv = -(K/(s tau + 1)^2)^{-1} (-K1/s tau1+1)
%    = K1/K (s tau + 1)^2/(s tau1 + 1) \approx K1/K 
Fv2 = K1/K;
