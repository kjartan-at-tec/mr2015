...
\label{sec:org1ec931e}
\begin{frame}[label={sec:org12bcafd}]{Second-order models}
\end{frame}
\begin{frame}[label={sec:orga3451a2}]{Two first-order models in series}
\begin{center}
\begin{tikzpicture}
...
\begin{frame}[label={sec:org304f223}]{Fitting second-order critically-damped model}
...
\label{sec:org4283999}
\begin{frame}[label={sec:orge4c874f}]{Feedback control}
   \begin{center}
   \begin{tikzpicture}[node distance=22mm, block/.style={rectangle, draw, minimum width=15mm}, sumnode/.style={circle, draw, inner sep=2pt}]
  { 
  \node[coordinate] (input) {};
...
\begin{frame}[label={sec:org4dd2674}]{The PID controller}
\begin{center}
...
\begin{frame}[label={sec:org0922ab0}]{The PID - Parallel form}
\definecolor{ppc}{rgb}{0.1,0.1,0.6}
\definecolor{iic}{rgb}{0.6,0.1,0.1}
\definecolor{ddc}{rgb}{0.1,0.6,0.1}
...
\begin{frame}[label={sec:org2568131}]{The PID - Parallel form, modified D-part}
\definecolor{ppc}{rgb}{0.1,0.1,0.6}
\definecolor{iic}{rgb}{0.6,0.1,0.1}
\definecolor{ddc}{rgb}{0.1,0.6,0.1}
...
\begin{frame}[label={sec:org1c505df}]{The PID - Parallel form}
\begin{center}
...
\begin{frame}[label={sec:org255e88e}]{The PID - Parallel form, solution}
\end{frame}
\begin{frame}[label={sec:org1a8df48}]{The PID - Parallel form, solution}
\(u(t) = k_c\Big( \textcolor{ppc}{e(t)} + \textcolor{iic}{\overbrace{\frac{1}{\tau_i} \int_0^{t} e(\xi) d\xi}^{u_I(t)}} + \textcolor{ddc}{ \underbrace{\tau_d \frac{d}{dt} \big(-y(t)\big)}_{u_D(t)}} \Big)\)
   \begin{center}
   \def\TT{1}
   \begin{tikzpicture}
...
\begin{frame}[label={sec:orge3a5031}]{The PID - practical form}
\definecolor{ppc}{rgb}{0.1,0.1,0.6}
\definecolor{iic}{rgb}{0.6,0.1,0.1}
...
\begin{frame}[label={sec:org7c2fc0d}]{Analytical PID tuning}
\end{frame}
\begin{frame}[label={sec:org7e773c4}]{Analytical PID tuning}
   \begin{center}
   \begin{tikzpicture}[node distance=22mm, block/.style={rectangle, draw, minimum width=15mm}, sumnode/.style={circle, draw, inner sep=2pt}]
  { 
  \node[coordinate] (input) {};
...
\begin{frame}[label={sec:orga2b1e7b}]{Analytical PID tuning - Solution}
\end{frame}
\begin{frame}[label={sec:orge752199}]{Analytical PID tuning - Solution}
...
\begin{frame}[label={sec:orgf8652c9}]{Analytic PID tuning - first-order with delay}
   \begin{center}
   \begin{tikzpicture}[node distance=22mm, block/.style={rectangle, draw, minimum width=15mm}, sumnode/.style={circle, draw, inner sep=2pt}]
  { 
  \node[coordinate] (input) {};
...
\[ F(s) = \frac{1}{K} \left( \frac{\tau s + 1}{\tau_c s + 1 - \mathrm{e}^{-s\theta}} \right) \approx \frac{1}{K} \left( \frac{\tau s + 1}{\tau_c s}\right)  = \underbrace{\frac{\tau}{K\tau_c}}_{k_c} \left( 1 + \frac{1}{\underbrace{\tau}_{\tau_i} s} \right).\]
Which is a PI-controller with \(k_c = \frac{\tau}{K\tau_c}\) and \(\tau_i = \tau\).
\end{frame}


\begin{frame}[label={sec:org64eaf66}]{SIMC-PID tuning rule}
