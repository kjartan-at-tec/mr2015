...
\label{sec:org6b44978}
\begin{frame}[label={sec:org862302b}]{Fitting first-order model with delay}
...
\label{sec:org6304eb7}
\begin{frame}[label={sec:org9ebde9e}]{Second-order models}
\end{frame}
\begin{frame}[label={sec:org127aaa3}]{Two first-order models in series}
\begin{center}
\begin{tikzpicture}
...
\begin{frame}[label={sec:orgb2323eb}]{Fitting second-order critically-damped model}
...
\begin{frame}[label={sec:org17e5fa4}]{Fitting second-order critically-damped model}
\alert{Model with two identical time-constants.}
Assuming model 
\[ \textcolor{green!50!black}{Y(s)} = \frac{K}{(s\tau + 1)^2}\textcolor{blue!80!black}{U(s)} \quad \overset{U(s) = \frac{u_f}{s}}{\Longrightarrow} \quad \textcolor{green!50!black}{y(t)} = u_f K\Big( 1 - (1+\frac{t}{\tau}\big)\mathrm{e}^{-\frac{t}{\tau}}\Big)u_H(t)\]
\def\Tcnst{2}
...
\label{sec:orga95b64a}

\begin{frame}[label={sec:org4a5124b}]{Second-order under-damped models}
A system with ODE
...
\begin{frame}[label={sec:orgb24c8c1}]{Second-order under-damped models}
...
\begin{frame}[label={sec:orge48eb0e}]{Second-order under-damped models}
\begin{columns}
\begin{column}{0.3\columnwidth}
\begin{center}
    \includegraphics[width=4cm]{../../figures/implane-second-order-poles}
...
\begin{frame}[label={sec:org3381a1f}]{Second-order under-damped models}
